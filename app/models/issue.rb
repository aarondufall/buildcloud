class Issue < ActiveRecord::Base
  attr_accessible :assigned_to_id, :closed_at, :description, :title, :item_id, :issue_type, :created_by_id, :email_tokens
  belongs_to :item
  ...
  def emails=(value)
    self.recipients = recipients.from_emails(Array(value), project)
    @emails = recipients.map(&:email)
  end
